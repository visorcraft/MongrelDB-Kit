...
- a synchronous **query builder** (CRUD, predicates, ordering, projections, aggregates, joins,
  group/having, subqueries, CTEs),
...
- **auto-increment sequences**, defaults, and a stable error taxonomy.
...
`mongreldb` (the native engine binding) is a peer dependency. Node 22+ is required. See the
[TypeScript guide](./typescript.md) for build/runtime details, and [Rust](./rust.md) /
