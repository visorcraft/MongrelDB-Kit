...
class Database:
    ...
    def create_user(self, username: str, password: str) -> None:
        ...
        self._handle.create_user(username, password)

    def drop_user(self, username: str) -> None:
        """Drop a user by username."""
        self._handle.drop_user(username)

    def alter_user_password(self, username: str, new_password: str) -> None:
        """Change a user's password."""
        self._handle.alter_user_password(username, new_password)

    def verify_user(self, username: str, password: str) -> bool:
        """Verify credentials. Returns True on success."""
        return self._handle.verify_user(username, password)

    def set_user_admin(self, username: str, is_admin: bool) -> None:
        """Grant or revoke admin privileges."""
        self._handle.set_user_admin(username, is_admin)

    def users(self) -> list[str]:
        """List all usernames."""
        return self._handle.users()

    def create_role(self, name: str) -> None:
        """Create a role."""
        self._handle.create_role(name)

    def drop_role(self, name: str) -> None:
        """Drop a role."""
        self._handle.drop_role(name)

    def roles(self) -> list[str]:
        """List all role names."""
        return self._handle.roles()

    def grant_role(self, username: str, role_name: str) -> None:
        """Grant a role to a user."""
        self._handle.grant_role(username, role_name)

    def revoke_role(self, username: str, role_name: str) -> None:
        """Revoke a role from a user."""
        self._handle.revoke_role(username, role_name)

    def grant_permission(self, role_name: str, permission: str) -> None:
        """Grant a permission to a role.

        Permission format: ``"all"``, ``"ddl"``, ``"admin"``, or
        ``"select:table"``, ``"insert:table"``, ``"update:table"``,
        ``"delete:table"``.
        """
        self._handle.grant_permission(role_name, permission)

    def revoke_permission(self, role_name: str, permission: str) -> None:
        """Revoke a permission from a role."""
        self._handle.revoke_permission(role_name, permission)
...
# ── user/role/credentials management ─────────────────────────────────
# These are added to the Database class above via _handle methods.

# The native extension (_handle) already exposes:
# create_user, drop_user, alter_user_password, verify_user,
# set_user_admin, users, create_role, drop_role, roles,
